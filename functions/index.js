...
export const githubProxy = functions.region('us-central1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const [repoRes, prsRes, issuesRes] = await Promise.all([
      ...
      fetch(`https://api.github.com/search/issues?q=repo:${repo}+type:issue+state:open&per_page=1`, { headers }),
    ]);
...
export const deleteAuthUser = functions.region('us-central1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (deleteFirestore) {
      try { await db.collection('users').doc(String(userId)).delete(); } catch {}
    }
...
export const updateAuthUser = functions.region('us-central1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const auth = getAuth(app);
    const db = getFirestore(app);
    const { uid, email, displayName, password, disabled, phoneNumber, photoURL, customClaims, updateFirestore } = req.body || {};
    let userId = uid;
    if (!userId && email) {
      const u = await auth.getUserByEmail(String(email));
      userId = u.uid;
    ...
    if (updateFirestore && typeof updateFirestore === 'object') {
      try { await db.collection('users').doc(String(userId)).set(updateFirestore, { merge: true }); } catch {}
    }
    return res.status(200).json({ ok: true, user: { uid: u.uid, email: u.email, displayName: u.displayName } });
  } catch (err) {
    return res.status(500).json({ error: String(err && err.message ? err.message : err) });
  }
});
...
export const importUsersCsv = functions.region('us-central1').https.onRequest(async (req, res) => {
  ...
  try {
    ...
    const { bucket, file, createMissing = true, updateExisting = true, defaultClaims = {} } = req.body || {};
    ...
    for (const r of required) { if (!header.includes(r)) return res.status(400).json({ error: `Missing column: ${r}` }); }
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      try {
        let user;
        try { user = await auth.getUserByEmail(email); } catch {}
        if (!user && createMissing) {
          user = await auth.createUser({ email, displayName: obj.displayname || obj.display_name || obj.name, password: obj.password, disabled: obj.disabled === 'true' });
          created++;
        ...
        if (user) {
          ...
          const fsData = { email, displayName: user.displayName || obj.displayname || obj.name, role: claims.role || null, updatedAt: Date.now() };
...
export const importUsersFromCSV = functions.region('us-central1').storage.object().onFinalize(async (object) => {
  try {
    const name = object.name || '';
    const bucketName = object.bucket;
    const contentType = object.contentType || '';
    if (!name || !bucketName) return;
    if (!name.endsWith('.csv')) { console.log('[importUsersFromCSV] ignorando arquivo não-CSV', name); return; }
    if (!name.startsWith('csv-imports/')) { console.log('[importUsersFromCSV] ignorando caminho', name); return; }
    ...
    for (const raw of rows) {
      ...
      try {
        ...
        const disabled = typeof obj.disabled === 'string' ? (obj.disabled.toLowerCase() === 'true') : undefined;
        const phoneNumber = obj.phonenumber || obj.phone_number || undefined;
        if (!user) {
          user = await auth.createUser({ email, displayName, disabled, phoneNumber, password: obj.password });
          created++;
        } else {
          const payload = {};
        ...
        const fsData = { email, displayName: (displayName || user.displayName || null), role: claims.role || null, updatedAt: Date.now() };
        await db.collection('users').doc(user.uid).set(fsData, { merge: true });
      } catch (e) {
        failed++;
      }
    }
    console.log(`[importUsersFromCSV] ${name} -> created: ${created}, updated: ${updated}, failed: ${failed}`);
    return;
  } catch (err) {
...
export const deleteAuthUserCallable = functions.region('us-central1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'A função deve ser chamada enquanto autenticado.');
  }
  if (!isAdminContext(context)) {
    throw new functions.https.HttpsError('permission-denied', 'Apenas usuários administradores podem excluir outros usuários.');
  }
  const uidToDelete = data?.uid;
  if (!uidToDelete) {
    throw new functions.https.HttpsError('invalid-argument', 'O UID a ser excluído é obrigatório.');
  }
  try {
    const app = ensureAdmin();
    const auth = getAuth(app);
    ...
    if (deleteFirestore) {
      const db = getFirestore(app);
      try { await db.collection('users').doc(String(uidToDelete)).delete(); } catch {}
    }
    return { success: true, message: `Usuário ${uidToDelete} excluído.` };
  } catch (error) {
    console.error('Erro ao excluir usuário:', error);
    throw new functions.https.HttpsError('internal', 'Não foi possível excluir o usuário.', error?.message);
  }
});

// Callable: updateAuthUser
export const updateAuthUserCallable = functions.region('us-central1').https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'A função deve ser chamada enquanto autenticado.');
  }
  const uidToUpdate = data?.uid;
  if (!uidToUpdate) {
    throw new functions.https.HttpsError('invalid-argument', 'O UID a ser atualizado é obrigatório.');
  }
  const isAdmin = isAdminContext(context);
  if (!isAdmin && context.auth.uid !== uidToUpdate) {
    throw new functions.https.HttpsError('permission-denied', 'Você não tem permissão para atualizar este usuário.');
  }
  const { email, password, displayName, photoURL, disabled, phoneNumber, customClaims, updateFirestore } = data || {};
  const updateData = {};
  ...
  if (Object.keys(updateData).length === 0 && !(customClaims && isAdmin) && !(updateFirestore && isAdmin)) {
    throw new functions.https.HttpsError('invalid-argument', 'Nenhum campo válido fornecido para atualização.');
  }
  try {
    const app = ensureAdmin();
    const auth = getAuth(app);
    ...
    if (isAdmin && updateFirestore && typeof updateFirestore === 'object') {
      const db = getFirestore(app);
      try { await db.collection('users').doc(String(uidToUpdate)).set(updateFirestore, { merge: true }); } catch {}
    }
    return { success: true, message: `Usuário ${uidToUpdate} atualizado.`, user: { uid: u.uid, email: u.email, displayName: u.displayName } };
  } catch (error) {
    console.error('Erro ao atualizar usuário:', error);
    throw new functions.https.HttpsError('internal', 'Não foi possível atualizar o usuário.', error?.message);
  }
});