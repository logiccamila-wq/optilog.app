...
const initialGroups: Group[] = [
  {
    ...
    items: [
      ...
      {
        id: 'traje',
        label: 'Traje mínimo: Calça, Camisa e Botina de Segurança?',
        critical: true,
        choice: null,
      },
      {
        id: 'condicoes',
        label: 'Condições físicas adequadas? (sem cansaço ou sono)',
        critical: true,
        choice: null,
      },
    ],
  },
  {
    ...
    items: [
      ...
      {
        id: 'cte',
        label: 'Possui CTe (Conhecimento de Transporte)?',
        critical: true,
        choice: null,
      },
  ...
  },
  {
    ...
    items: [
      ...
      {
        id: 'calcos',
        label: 'Veículo possui os dois calços de roda?',
        critical: true,
        choice: null,
      },
      {
        id: 'rodas',
        label: 'Condição das rodas, pneus e estepes estão em bom estado?',
        critical: true,
        choice: null,
      },
      {
        id: 'sinais',
        label: 'Faróis, buzina, lanternas e setas em funcionamento?',
        critical: true,
        choice: null,
      },
      {
        id: 'cinto',
        label: 'Cinto de segurança de 3 pontos em bom estado?',
        critical: true,
        choice: null,
      },
      {
        id: 'compartimento',
        label: 'Compartimento de carga em bom estado?',
        critical: false,
        choice: null,
      },
    ],
  },
  {
    ...
    items: [
      {
        id: 'extintor',
        label: 'Possui Extintor de incêndio adequado?',
        critical: true,
        choice: null,
      },
      {
        id: 'paineis',
        label: 'Painéis de segurança de acordo com o produto (ONU/Risco)?',
        critical: true,
        choice: null,
      },
      {
        id: 'rotulos',
        label: 'Rótulos de risco de acordo com o produto?',
        critical: true,
        choice: null,
      },
  ...
  },
  {
    ...
    items: [
      {
        id: 'perigoso',
        label: 'O carregamento possui produto perigoso/químico?',
        critical: true,
        choice: null,
      },
      {
        id: 'tanques',
        label: 'Tanques, válvulas e conexões sem vazamento e corrosão?',
        critical: true,
        choice: null,
      },
      {
        id: 'embalagens',
        label: 'Embalagens em perfeitas condições, sem vazamento?',
        critical: true,
        choice: null,
      },
    ],
    ...
    <section style={{ border: '1px solid #333', borderRadius: 12, padding: 16 }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <div>
          ...
          <small style={{ color: '#999' }}>
            Preencha cada item. Qualquer item <strong>NÃO</strong> crítico gera alerta. Itens{' '}
            <strong>NÃO</strong> críticos de segurança (⭐) impedem a liberação.
          </small>
        </div>
        <div style={{ textAlign: 'right' }}>
          ...
          <small style={{ color: '#999' }}>
            Itens Verificados: {stats.verified} | Não Conformidades: {stats.nonConformities}
          </small>
        </div>
      </header>
              ...
              {g.items.map((it) => (
                ...
                >
                  <div style={{ color: '#ddd' }}>
                    {it.label}{' '}
                    {it.critical && (
                      <span title="Crítico" style={{ color: '#f59e0b' }}>
                        ⭐
                      </span>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button
                      onClick={() => setChoice(g.id, it.id, 'SIM')}
                      style={pill(it.choice === 'SIM', '#16a34a')}
                    >
                      SIM
                    </button>
                    <button
                      onClick={() => setChoice(g.id, it.id, 'NAO')}
                      style={pill(it.choice === 'NAO', '#ef4444')}
                    >
                      NÃO
                    </button>
                    <button
                      onClick={() => setChoice(g.id, it.id, 'NA')}
                      style={pill(it.choice === 'NA', '#64748b')}
                    >
                      N.A.
                    </button>
                  </div>
      ...
      <div style={{ marginTop: 16 }}>
        ...
        <p style={{ color: '#bbb', marginTop: 0 }}>
          Declaro que o veículo foi inspecionado, que recebi a documentação e as informações sobre
          os riscos do produto a ser transportado.
        </p>
        <div
