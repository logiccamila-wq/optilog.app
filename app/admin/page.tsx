    ...
    <div style={{ maxWidth: 1000, margin: '24px auto', padding: 16 }}>
      ...
      {!hasToken && (
        <div style={{ marginTop: 12, padding: 8, border: '1px solid #444', borderRadius: 6 }}>
          Você precisa estar autenticado via JWT. Faça login em{' '}
          <a href="/jwt/login">/jwt/login</a>.
          <div style={{ marginTop: 8 }}>
            <button onClick={() => (window.location.href = '/jwt/login')} style={{ padding: '6px 12px' }}>
              Ir para login
            </button>
          </div>
        </div>
      ...
      <section id="update-user" style={{ marginTop: 24, padding: 16, border: '1px solid #333', borderRadius: 8 }}>
      ...
      <section style={{ marginTop: 24, padding: 16, border: '1px solid #333', borderRadius: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => fetchUsers()} disabled={!hasToken || loadingList} style={{ padding: '6px 12px' }}>
            {loadingList ? 'Carregando...' : 'Atualizar'}
          </button>
        </div>

        {errorList && (
          <div style={{ marginTop: 8, color: '#c55' }}>Erro: {errorList}</div>
        )}
        ...
        <div style={{ marginTop: 12, overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>UID</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Email</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Nome</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Role</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Telefone</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Status</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Criado</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Atualizado</th>
                <th style={{ textAlign: 'left', borderBottom: '1px solid #333', padding: 6 }}>Ações</th>
              </tr>
            </thead>
            <tbody>
              {users.map((u) => (
                <tr key={u.uid}>
                  ...
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{u.display_name || '-'}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{u.role || '-'}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{u.phone_number || '-'}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{u.disabled ? 'Desabilitado' : 'Ativo'}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{fmtDate(u.created_at)}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>{fmtDate(u.updated_at)}</td>
                  <td style={{ borderBottom: '1px solid #222', padding: 6 }}>
                    <button onClick={() => copyUid(u.uid)} style={{ padding: '4px 8px', marginRight: 6 }}>Copiar UID</button>
                    <button onClick={() => quickEdit(u)} style={{ padding: '4px 8px' }}>Editar</button>
                  </td>
                </tr>
              ))}
              {users.length === 0 && !loadingList && (
                <tr>
                  <td colSpan={9} style={{ padding: 8, color: '#aaa' }}>Nenhum usuário encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
