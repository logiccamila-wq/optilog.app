...
export async function GET(req: NextRequest) {
  try {
    ...
    if (q) whereClauses.push(sql`(vehicle_type ILIKE ${'%' + q + '%'} OR axle_config ILIKE ${'%' + q + '%'} OR fuel_type ILIKE ${'%' + q + '%'} OR source ILIKE ${'%' + q + '%'})`);
    ...
    const totalRows = await sql`SELECT count(*)::int as total FROM vehicle_consumption_params ${where}`;
    const total = Array.isArray(totalRows) ? (totalRows[0]?.total ?? 0) : (totalRows?.[0]?.total ?? 0);

    const rows = await sql`SELECT id, vehicle_type, axle_config, fuel_type, avg_consumption_kml, source, effective_date, limits, created_at, updated_at
...
export async function POST(req: NextRequest) {
  try {
    ...
    const avg_consumption_kml = body?.avg_consumption_kml != null && body.avg_consumption_kml !== '' ? parseFloat(String(body.avg_consumption_kml)) : null;
    const source = (body?.source || '').trim();
    const effective_date = (body?.effective_date || '').trim();
    const limits = body?.limits ?? null;

    if (!vehicle_type) return NextResponse.json({ error: 'vehicle_type é obrigatório' }, { status: 400 });
    if (!axle_config) return NextResponse.json({ error: 'axle_config é obrigatório' }, { status: 400 });
    if (!fuel_type) return NextResponse.json({ error: 'fuel_type é obrigatório' }, { status: 400 });
    if (avg_consumption_kml !== null && avg_consumption_kml <= 0) return NextResponse.json({ error: 'avg_consumption_kml deve ser > 0' }, { status: 400 });

    const sql = getSql();
    const rows = await sql`INSERT INTO vehicle_consumption_params (vehicle_type, axle_config, fuel_type, avg_consumption_kml, source, effective_date, limits)
    ...
    return NextResponse.json({ ok: true, record: Array.isArray(rows) ? rows[0] : rows?.[0] }, { status: 201 });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'Erro interno' }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ ok: true, record: Array.isArray(rows) ? rows[0] : rows?.[0] }, { status: 200 });
  } catch (err: any) {
    return NextResponse.json({ error: err?.message || 'Erro interno' }, { status: 500 });
  }
}
