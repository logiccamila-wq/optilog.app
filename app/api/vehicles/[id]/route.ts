...
async function ensureSchema(sql: any) {
  ...
  await sql(
    'alter table if exists vehicles add column if not exists updated_at timestamptz default now()'
  );
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const rows =
      await sql`select id, plate, model, year, odometer, chassis, renavam, color, brand, cost_center, goal_desc, axles_count, axle_config_name, axle_weights, gross_weight_estimated, tire_type, tire_dimensions, purchase_value, ownership, created_at, updated_at from vehicles where id = ${id}`;
    const vehicle = Array.isArray(rows) ? rows[0] : rows?.[0];
    return NextResponse.json(vehicle, { status: 200 });
  } catch (err: any) {
    return NextResponse.json(
      { error: 'Erro ao atualizar veículo', detail: err?.message || String(err) },
      { status: 500 }
    );
  }
}
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(
      { error: 'Erro ao buscar veículo', detail: err?.message || String(err) },
      { status: 500 }
    );
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    try {
      await publishEvent({ entity: 'vehicle', action: 'delete', data: deleted });
    } catch {}
