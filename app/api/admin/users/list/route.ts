...
export async function GET(req: NextRequest) {
  try {
    const auth = await extractBearer(req);
    if (!auth) {
      return NextResponse.json({ ok: false, error: 'missing_bearer' }, { status: 401 });
    }
    const tokenResult = await verifyToken(auth);
    const payload = (tokenResult?.payload as Record<string, any>) || {};
    ...
    if (search) {
      whereClauses.push(
        `(email ILIKE $${params.length + 1} OR display_name ILIKE $${params.length + 2})`
      );
