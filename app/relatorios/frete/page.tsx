    ...
    <main className="container" style={{ display: 'grid', gap: 12 }}>
      ...
      <Paper sx={{ p: 2 }} variant="outlined">
        <Typography variant="caption">
          Metodologia: Sicro2/CONAB (parâmetros configuráveis)
        </Typography>
      </Paper>
        ...
        <Paper sx={{ p: 2 }} variant="outlined">
          ...
          {params ? (
            <Box sx={{ display: 'grid', gap: 1 }}>
              ...
              <Button onClick={saveParams} disabled={loading} variant="outlined">
        ...
        <Paper sx={{ p: 2 }} variant="outlined">
          ...
          <Box sx={{ display: 'grid', gap: 1 }}>
            ...
            <TextField
              ...
              type="number"
              value={payloadTon}
              onChange={(e) => setPayloadTon(Number(e.target.value))}
              size="small"
            />
            <TextField
              label="Pedágio (R$/km)"
              type="number"
              value={tollsPerKm ?? ''}
              onChange={(e) => setTollsPerKm(Number(e.target.value))}
              size="small"
              placeholder="usar default"
            />
            <TextField
              label="Rateio fixos (mês)"
              type="number"
              value={monthsEq}
              onChange={(e) => setMonthsEq(Number(e.target.value))}
              size="small"
            />
            <TextField
              label="Veículo (id)"
            ...
            <>
              ...
              >
                <Paper sx={{ p: 1 }} variant="outlined">
                  Custo por km: R$ {breakdown.totals.cost_per_km.toFixed(2)}
                </Paper>
                <Paper sx={{ p: 1 }} variant="outlined">
                  Custo por ton-km: R$ {breakdown.totals.cost_per_ton_km.toFixed(4)}
                </Paper>
                <Paper sx={{ p: 1 }} variant="outlined">
                  Custo da viagem: R$ {breakdown.totals.cost_trip.toFixed(2)}
                </Paper>
                <Paper sx={{ p: 1 }} variant="outlined">
                  Frete (com margem): R$ {breakdown.totals.freight_with_margin.toFixed(2)}
                </Paper>
              </Box>
