...
export default function RelatorioCapacidadePage() {
  ...
  const TOL_AXLE = 0.1; // 10% por eixo

  const load = async () => {
    setLoading(true);
    try {
      const res = await fetch(
        `/api/vehicles?page=1&pageSize=1000${q ? `&q=${encodeURIComponent(q)}` : ''}`,
        { cache: 'no-store' }
      );
  ...
  const totals = useMemo(() => {
    ...
    const compliantGross = list.filter((v) => {
      ...
      const gross =
        v.gross_weight_estimated ?? axlePresets.find((p) => p.name === name)?.gross ?? null;
  ...
  return (
    <main className="container" style={{ display: 'grid', gap: 12 }}>
      ...
      {groups.map((g) => {
        const mismatch = g.items.filter(
          (v: any) =>
            typeof v.axles_count === 'number' &&
            g.expectedAxles !== null &&
            v.axles_count !== g.expectedAxles
        );
        return (
            ...
            <Table size="small">
              ...
              <TableBody>
                {g.items.map((v: any) => {
                  ...
                  const gross =
                    v.gross_weight_estimated ??
                    axlePresets.find((p) => p.name === v.axle_config_name)?.gross ??
                    null;
                  const status = (() => {
