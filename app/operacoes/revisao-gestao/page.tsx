  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-sm text-gray-600">
        Padronize suas reuniões de revisão, garanta conformidade e foque em melhoria contínua.
      </p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1 text-sm">
              <input
                type="checkbox"
                checked={meeting.scope.q9001}
                onChange={(e) =>
                  setMeeting({ ...meeting, scope: { ...meeting.scope, q9001: e.target.checked } })
                }
              />
              9001
            </label>
            <label className="flex items-center gap-1 text-sm">
              <input
                type="checkbox"
                checked={meeting.scope.e14001}
                onChange={(e) =>
                  setMeeting({ ...meeting, scope: { ...meeting.scope, e14001: e.target.checked } })
                }
              />
              14001
            </label>
            <label className="flex items-center gap-1 text-sm">
              <input
                type="checkbox"
                checked={meeting.scope.s45001}
                onChange={(e) =>
                  setMeeting({ ...meeting, scope: { ...meeting.scope, s45001: e.target.checked } })
                }
              />
              45001
            </label>
      ...
      <section className="border p-4 rounded">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            ...
            onClick={() => setAgenda([...agenda, { title: 'Novo item' }])}
          >
            Adicionar item
          </button>
        </div>
        <div className="space-y-3">
          {agenda.map((it, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-4 gap-2">
              ...
              <input
                ...
                onChange={(e) => {
                  const a = [...agenda];
                  a[idx] = { ...it, dueDate: e.target.value };
                  setAgenda(a);
                }}
              />
              <button
              ...
              <textarea
                className="md:col-span-4 border rounded p-2"
                rows={2}
                placeholder="Descrição"
                value={it.description ?? ''}
                onChange={(e) => {
                  const a = [...agenda];
                  a[idx] = { ...it, description: e.target.value };
                  setAgenda(a);
                }}
              />
            </div>
          ...
          {agenda.length === 0 && (
            <p className="text-sm text-gray-600">Adicione tópicos a serem discutidos.</p>
          )}
        </div>
      </section>
          ...
          {entradas.map((en, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <textarea
                className="border rounded p-2"
                rows={2}
                placeholder="Resumo, dados e conclusões"
                value={en.details}
                onChange={(e) => {
                  const x = [...entradas];
                  x[idx] = { ...en, details: e.target.value };
                  setEntradas(x);
                }}
              />
            </div>
      ...
      <section className="border p-4 rounded">
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() =>
                setAcoes([
                  ...acoes,
                  { title: 'Nova ação', responsible: '', deadline: '', status: 'Planejada' },
                ])
              }
            >
              Adicionar ação
            </button>
            <button className="px-3 py-1 border rounded" onClick={exportAcoesCSV}>
              Exportar CSV
            </button>
          </div>
        </div>
        <div className="space-y-3">
          {acoes.map((ac, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-5 gap-2">
              ...
              <input
                ...
                onChange={(e) => {
                  const x = [...acoes];
                  x[idx] = { ...ac, responsible: e.target.value };
                  setAcoes(x);
                }}
              />
              <input
                ...
                onChange={(e) => {
                  const x = [...acoes];
                  x[idx] = { ...ac, deadline: e.target.value };
                  setAcoes(x);
                }}
              />
              <select
                ...
                onChange={(e) => {
                  const x = [...acoes];
                  x[idx] = { ...ac, status: e.target.value as ActionItem['status'] };
                  setAcoes(x);
                }}
              >
          ...
          {acoes.length === 0 && (
            <p className="text-sm text-gray-600">Inclua ações com responsável, prazo e status.</p>
          )}
        </div>
      </section>
      ...
      <section className="border p-4 rounded">
        <div className="flex items-center justify-between mb-2">
          ...
          <button
            className="px-3 py-1 border rounded"
            onClick={() => setKpis([...kpis, { name: 'Novo KPI', target: '', current: '' }])}
          >
            Adicionar KPI
          </button>
        </div>
        <div className="space-y-3">
          {kpis.map((k, idx) => (
            <div key={idx} className="grid grid-cols-1 md:grid-cols-4 gap-2">
              ...
              <input
                ...
                onChange={(e) => {
                  const x = [...kpis];
                  x[idx] = { ...k, current: e.target.value };
                  setKpis(x);
                }}
              />
              <input
          ...
          {kpis.length === 0 && (
            <p className="text-sm text-gray-600">Cadastre KPIs de qualidade, ambiental e SSO.</p>
          )}
        </div>
      </section>
      ...
      <section className="border p-4 rounded">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <input
            ...
            value={turtle.process}
            onChange={(e) => setTurtle({ ...turtle, process: e.target.value })}
          />
          <input
            className="border rounded p-2"
            placeholder="Entradas"
            value={turtle.inputs}
            onChange={(e) => setTurtle({ ...turtle, inputs: e.target.value })}
          />
          <input
            className="border rounded p-2"
            placeholder="Saídas"
            value={turtle.outputs}
            onChange={(e) => setTurtle({ ...turtle, outputs: e.target.value })}
          />
          <input
            className="border rounded p-2"
            placeholder="Recursos"
            value={turtle.resources}
            onChange={(e) => setTurtle({ ...turtle, resources: e.target.value })}
          />
          <input
            className="border rounded p-2"
            placeholder="Controles"
            value={turtle.controls}
            onChange={(e) => setTurtle({ ...turtle, controls: e.target.value })}
          />
          <input
            className="border rounded p-2"
            placeholder="Métricas"
            value={turtle.metrics}
            onChange={(e) => setTurtle({ ...turtle, metrics: e.target.value })}
          />
        </div>
      </section>
      ...
      <section className="flex gap-2">
        <button className="px-3 py-2 border rounded" onClick={exportAta}>
          Exportar Ata (JSON)
        </button>
        <button className="px-3 py-2 border rounded" onClick={exportAcoesCSV}>
          Exportar Plano de Ações (CSV)
        </button>
      </section>

      <p className="text-xs text-gray-500">
        Conformidade: ISO 9001/14001/45001 — Revisão pela Direção (9.3). Registre decisões e ações
        com evidência e acompanhe KPIs para melhoria contínua.
      </p>
    </div>
  );
}
