...
function usePing(url?: string) {
  ...
  const onPing = async () => {
    if (!url) {
      setStatus('fail');
      setMessage('URL não configurada');
      return;
    }
    try {
  ...
  return (
    <main className="container" style={{ padding: '2rem', maxWidth: 960 }}>
      ...
      <section style={{ marginTop: '2rem' }}>
        ...
        >
          <div style={{ border: '1px solid #333', borderRadius: 8, padding: '1rem' }}>
            <h3>n8n</h3>
            <p>
              Base: <code>{appConfig.integration?.n8n?.baseUrl || 'não configurado'}</code>
            </p>
            <button onClick={pingN8n} style={{ padding: '0.5rem 1rem' }}>
              Testar Health
            </button>
            <p>
              Status: <strong>{n8nStatus}</strong> {n8nMsg && `- ${n8nMsg}`}
            </p>
          </div>
          <div style={{ border: '1px solid #333', borderRadius: 8, padding: '1rem' }}>
            <h3>Zoho Mail</h3>
            <p>
              API: <code>{appConfig.integration?.zohoMail?.apiBaseUrl || 'não configurado'}</code>
            </p>
            <p>
              Token presente: <strong>{zohoConfigured ? 'sim' : 'não'}</strong>
            </p>
          ...
          <div style={{ border: '1px solid #333', borderRadius: 8, padding: '1rem' }}>
            <h3>SEFAZ (CT-e)</h3>
            <p>
              Ambiente: <code>{appConfig.integration?.sefaz?.env || 'não definido'}</code>
            </p>
            <p>
              Endpoint: <code>{appConfig.integration?.sefaz?.cteBaseUrl || 'não configurado'}</code>
            </p>
            <p>
              Configuração: <strong>{sefazConfigured ? 'presente' : 'ausente'}</strong>
            </p>
      ...
      <section style={{ marginTop: '2rem' }}>
        ...
        <button onClick={pingApi} style={{ padding: '0.5rem 1rem' }}>
          Ping /api/health
        </button>
        <p>
          Status: <strong>{apiStatus}</strong> {apiMsg && `- ${apiMsg}`}
        </p>
      </section>
      ...
      <section style={{ marginTop: '2rem' }}>
        ...
        <ol>
          <li>
            Abra <strong>Configurações</strong> do aparelho.
          </li>
          <li>
            Acesse <strong>Sobre o telefone</strong> e toque 7x em{' '}
            <strong>Número de compilação</strong>.
          </li>
          <li>
            Digite seu PIN se solicitado; as <strong>Opções do desenvolvedor</strong> aparecerão.
          </li>
        </ol>
        <p>
          <strong>Precauções:</strong> alterações incorretas podem afetar desempenho e estabilidade.
        </p>
      </section>
      ...
      <section style={{ marginTop: '2rem' }}>
        ...
        <p>
          Para simular localização no navegador, use as DevTools (Sensors) do Chrome. Você também
          pode passar <code>?mockLocation=lat,lng</code> em URLs internas e ler esse valor no app.
        </p>
      </section>
    </main>
  );
